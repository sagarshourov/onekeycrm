import { Lucide, Modal, ModalBody, ModalFooter } from "@/base-components";

import { useState } from "react";

import { useRecoilValue, useRecoilStateLoadable } from "recoil";
import { assignSelect, allUserListState } from "../../state/admin-atom";
...
function getEmployees(array) {
  return filter(array, (item) => item.is_admin === 3);
}
...
const UsersView = (props) => {
  ...
  let admin = getSingleUser(usersData.contents, parseInt(id));

  let employees = getEmployees(usersData.contents);

  const assignAdminUser = async () => {
    const URL = adminApi() + "assign_employee";
  ...
  const deleteAssign = async () => {
    const URL = adminApi() + "del_assign_employee";
  ...
  return (
    <>
      ...
      >
        <ModalBody className="p-0">
          <h3 className="text-3xl p-5 text-center">Select Employee</h3>
