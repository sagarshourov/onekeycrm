...
import { SlackUsersState, SlackConvrsionList } from "../../state/slack-state";
...
function conversionFilters(array) {
  return filter(array, (_items) => {
...
const AdminUsers = (props) => {
  ...
  const [slackUsers, setData] = useRecoilStateLoadable(SlackUsersState);


  
  const [slackConList, setSlackChannel] =
    useRecoilStateLoadable(SlackConvrsionList);

  var bot = new SlackB({ token: process.env.token });
  //console.log("user data", slackUsers);
  ...
  const inviteUser = async (e) => {
    ...
    var email = Object.fromEntries(data).email;
  ...
  let filterData = applySortFilters(slackUsers.contents, search);
  let empData = empFilters(usersData.contents);

  let conData = conversionFilters(slackConList.contents);

  
  return (
    <>
      <h2 className="intro-y text-lg font-medium mt-10 ">Slack Users List</h2>
      <div className=" mt-5">
        <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
          ...
          <div className="hidden md:block mx-auto text-slate-500">
            ...
            {slackUsers.state === "hasValue" && slackUsers.contents["length"]}
          </div>
      ...
      >
        <ModalBody className="p-0">
          <form onSubmit={(e) => inviteUser(e)}>
            <div className="p-5 text-center">
              <div className="text-2xl mt-5 mb-5">Assign  user</div>
              <div className="p-5">
                <div className="form-inline">
                  ...
                  <select name="email" className="form-control">
