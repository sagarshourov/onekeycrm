...
const EmpFollowUpReport = (props) => {
  // let { id } = useParams();
  const [deleteConfirmationModal, setDeleteConfirmationModal] = useState(false);

  const [callData, setCallState] = useRecoilStateLoadable(
    empFollowUpReportSelect
  );
  ...
  const handelResult = (e) => {
    ...
    setResult(val);
    setAtomResult(val);
    setReportName(e.target.selectedOptions[0].text);
  };
                ...
                <Alert className="alert-dark mb-2 w-auto my-5">
                  <div className="flex items-center">
                    <div className="font-medium text-lg">
                      {reportName} Call Report ({daterange})
                    </div>
                  </div>
                </Alert>
              )}
...
export default EmpFollowUpReport;
